...
BATCH     = 64                # batch size
LR        = 5e-3              # learning rate (safer for aug)
...
def train_joint(model, X_th, y_th, X_z, y_z, epochs, batch, lr, outdir):
    ...
    if ds is None:
        ...
        return
    try:
        AdamOpt = keras.optimizers.legacy.Adam   # TF ≥ 2.11
    except AttributeError:
        AdamOpt = keras.optimizers.Adam          # TF ≤ 2.10
    opt = AdamOpt(learning_rate=lr, clipnorm=5.0)
    ...
    for ep in range(1, E + 1):
        ...
        # --- print LR in CLI ---
        try:
            # If LR is a schedule, evaluate at current global step
            cur_lr = float(opt.learning_rate(opt.iterations).numpy()) if callable(opt.learning_rate) else \
                     float(opt.learning_rate.numpy())                  if hasattr(opt.learning_rate, "numpy") else \
                     float(opt.learning_rate)
        except Exception:
            cur_lr = float(lr)  # safe fallback

        if (ep % 20) == 0 or ep <= 5:
            print(f"[joint] epoch {ep}  loss={avg:.5f}  (θ={avgT:.5f}, z={avgZ:.5f})  lr={cur_lr:.3e}")
        ...
        if avg + tol < best:
            ...
            stale += 1
            if stale >= patience:
                print(f"[joint] early stop at epoch {ep} (best={best:.5f})  lr={cur_lr:.3e}")
                break
