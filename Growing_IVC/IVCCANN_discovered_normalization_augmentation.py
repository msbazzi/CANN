...
LMIN = 0.6      # min stretch
LMAX = 2.0       # max stretch (very conservative)
EXP_CLIP = 15.0  # cap exp pre-activations to avoid overflow
INV_CLIP = 25.0  # cap residual invariants before feeding dense layers
...
NORM_STRESS = False                # normalize stresses per file during training
...
# ==== AUGMENTATION (safe defaults) ============================================
AUG_ENABLE        = False     # master switch
AUG_DUP_FACTOR    = 1        # how many augmented copies per set (0 or 1 is fine)
AUG_STRESS_NOISE  = 0.5      # kPa stdev added to labels (both θ and z)
AUG_LAMBDA_JITTER = 0.01     # 1% multiplicative jitter on λ
AUG_MIXUP_ALPHA   = 0.25     # Beta(alpha, alpha) for mixup (0 disables)
# =============================================================================
...
class PsiNet(keras.Model):
    ...
    def call(self, I1, I2, I4th, I4z, training=False):
        I1r = tf.clip_by_value(I1 - self.shift_I1, -INV_CLIP, INV_CLIP)
        I2r = tf.clip_by_value(I2 - self.shift_I2, -INV_CLIP, INV_CLIP)
        I4thr = tf.clip_by_value(I4th - self.shift_I4, -INV_CLIP, INV_CLIP)
        I4zr  = tf.clip_by_value(I4z  - self.shift_I4, -INV_CLIP, INV_CLIP)

        def safe_expm1(z):
            z = tf.clip_by_value(z, -EXP_CLIP, EXP_CLIP)
            return tf.math.expm1(z)
        def apply_branch(x, layers):
                # also bound x^2 indirectly by clipping x above
                t1 = layers[0](x)
                t2 = safe_expm1(layers[1](x))
                x2 = tf.square(x)  # bounded because x is clipped
                t3 = layers[2](x2)
                t4 = safe_expm1(layers[3](x2))
                # optional: clip the linear outputs to keep heads numerically tame
                t1 = tf.clip_by_value(t1, -1e3, 1e3)
                t3 = tf.clip_by_value(t3, -1e3, 1e3)
...
def _make_joint_dataset(X_th, y_th, X_z, y_z, batch, meta_th=None, meta_z=None):
    """
    Build one dataset containing both Pd (theta) and Fl (z) rows.
    All returned arrays have the SAME length N = Lth + Lz.
    meta_th/meta_z are dicts like {"ref": r, "scl": s}, with lengths Lth or Lz.
    """
    Lth = X_th.shape[0]
    Lz  = X_z.shape[0]
    N   = Lth + Lz
    ...
    # Stack inputs
    if Lth > 0 and Lz > 0:
        X = np.vstack([X_th, X_z]).astype(np.float32)
    elif Lth > 0:
        X = X_th.astype(np.float32)
    else:
        X = X_z.astype(np.float32)

    # Targets: theta available on first Lth rows, z available on last Lz rows
    if Lth > 0 and Lz > 0:
        ytheta = np.concatenate([y_th, np.full(Lz, np.nan, dtype=np.float32)], axis=0)
        yz     = np.concatenate([np.full(Lth, np.nan, dtype=np.float32), y_z], axis=0)
    elif Lth > 0:
        ytheta = y_th.astype(np.float32)
        yz     = np.full(Lth, np.nan, dtype=np.float32)
    else:
        ytheta = np.full(Lz, np.nan, dtype=np.float32)
        yz     = y_z.astype(np.float32)

    # --- Build FULL-LENGTH normalization vectors aligned with X ---
    # Pd normalization (theta): real values on first Lth rows, neutral on last Lz rows
    if Lth > 0:
        if meta_th is not None:
            rth_theta = np.asarray(meta_th["ref"], dtype=np.float32)
            sth_theta = np.asarray(meta_th["scl"], dtype=np.float32)
            if rth_theta.shape[0] != Lth or sth_theta.shape[0] != Lth:
                print(f"[WARN] meta_th length mismatch: got {rth_theta.shape[0]}/{sth_theta.shape[0]} vs Lth={Lth}. "
                      f"Using neutral values.")
                rth_theta = np.zeros(Lth, np.float32)
                sth_theta = np.ones(Lth,  np.float32)
        else:
            rth_theta = np.zeros(Lth, np.float32)
            sth_theta = np.ones(Lth,  np.float32)
    else:
        rth_theta = np.zeros(0, np.float32)
        sth_theta = np.ones(0,  np.float32)

    rth_full = np.concatenate([rth_theta, np.zeros(Lz, np.float32)], axis=0)
    sth_full = np.concatenate([sth_theta, np.ones( Lz, np.float32)], axis=0)

    # Fl normalization (z): neutral on first Lth rows, real values on last Lz rows
    if Lz > 0:
        if meta_z is not None:
            rz_z = np.asarray(meta_z["ref"], dtype=np.float32)
            sz_z = np.asarray(meta_z["scl"], dtype=np.float32)
            if rz_z.shape[0] != Lz or sz_z.shape[0] != Lz:
                print(f"[WARN] meta_z length mismatch: got {rz_z.shape[0]}/{sz_z.shape[0]} vs Lz={Lz}. "
                      f"Using neutral values.")
                rz_z = np.zeros(Lz, np.float32)
                sz_z = np.ones( Lz, np.float32)
        else:
            rz_z = np.zeros(Lz, np.float32)
            sz_z = np.ones( Lz, np.float32)
    else:
        rz_z = np.zeros(0, np.float32)
        sz_z = np.ones(0,  np.float32)

    rz_full = np.concatenate([np.zeros(Lth, np.float32), rz_z], axis=0)
    sz_full = np.concatenate([np.ones( Lth, np.float32), sz_z], axis=0)

    # --- FAST_DEV: subsample consistently across ALL arrays ---
    if FAST_DEV and N > MAX_POINTS_PER_AGE:
        sel = np.linspace(0, N - 1, MAX_POINTS_PER_AGE, dtype=int)
        X       = X[sel]
        ytheta  = ytheta[sel]
        yz      = yz[sel]
        rth_full= rth_full[sel]
        sth_full= sth_full[sel]
        rz_full = rz_full[sel]
        sz_full = sz_full[sel]

    # Final sanity check (all first dims must match)
    n = X.shape[0]
    for name, arr in dict(
        ytheta=ytheta, yz=yz,
        rth=rth_full, sth=sth_full,
        rz=rz_full,  sz=sz_full
    ).items():
        if arr.shape[0] != n:
            raise ValueError(f"[BUG] {name} length {arr.shape[0]} != X length {n}")

    ds = tf.data.Dataset.from_tensor_slices(
        (X.astype(np.float32),
         ytheta.astype(np.float32),
         yz.astype(np.float32),
         rth_full.astype(np.float32),
         sth_full.astype(np.float32),
         rz_full.astype(np.float32),
         sz_full.astype(np.float32))
    ).shuffle(min(10000, max(1000, n)), seed=SEED) \
     .batch(BATCH, drop_remainder=True) \
     .cache() \
     .prefetch(tf.data.AUTOTUNE)
    return ds
...
def _compute_joint_grads(model, xb, ytheta, yz, rth, sth, rz, sz):
    ...
    with tf.GradientTape() as tape_out:
        ...
        tf.debugging.assert_all_finite(I1, "I1 NaN/Inf")
        tf.debugging.assert_all_finite(I2, "I2 NaN/Inf")
        tf.debugging.assert_all_finite(dW_dI1, "dW_dI1 NaN/Inf")
        tf.debugging.assert_all_finite(dW_dI2, "dW_dI2 NaN/Inf")
        tf.debugging.assert_all_finite(dW_dI4th, "dW_dI4th NaN/Inf")
        tf.debugging.assert_all_finite(dW_dI4z, "dW_dI4z NaN/Inf")
        tf.debugging.assert_all_finite(sig_th_hat, "NaN/Inf in sigma_theta_hat")
        tf.debugging.assert_all_finite(sig_z_hat,  "NaN/Inf in sigma_z_hat")
...
def _rand_like(x, scale):
    return np.random.normal(loc=0.0, scale=scale, size=x.shape).astype(np.float32)

def augment_arrays(X, y, kind,
                   dup_factor=AUG_DUP_FACTOR,
                   stress_noise=AUG_STRESS_NOISE,
                   lambda_jitter=AUG_LAMBDA_JITTER,
                   mixup_alpha=AUG_MIXUP_ALPHA):
    """
    X: (N,2)  -> [lambda_theta, lambda_z]
    y: (N,)
    kind: 'theta' or 'z' (only used for logging)
    Returns augmented (X_aug, y_aug) with original data included first.
    """
    if X.shape[0] == 0 or (dup_factor <= 0 and mixup_alpha <= 0):
        return X, y

    X_list = [X.astype(np.float32)]
    y_list = [y.astype(np.float32)]

    # --- (A) duplicate with small jitter on λ and noise on σ ---
    for _ in range(max(0, dup_factor)):
        Xj = X.astype(np.float32).copy()
        if lambda_jitter > 0:
            # multiplicative jitter: λ' = λ * (1 + ε)
            jit = 1.0 + _rand_like(Xj, lambda_jitter)
            Xj = Xj * jit
            # clip to valid range so invariants remain sane
            Xj[:, 0] = np.clip(Xj[:, 0], LMIN, LMAX)  # λθ
            Xj[:, 1] = np.clip(Xj[:, 1], LMIN, LMAX)  # λz
        yj = y.astype(np.float32).copy()
        if stress_noise > 0:
            yj = yj + np.random.normal(0.0, stress_noise, size=yj.shape).astype(np.float32)
        X_list.append(Xj); y_list.append(yj)

    # --- (B) mixup augmentation ---
    if mixup_alpha and mixup_alpha > 0 and X.shape[0] >= 2:
        lam = np.random.beta(mixup_alpha, mixup_alpha, size=X.shape[0]).astype(np.float32)
        idx = np.random.permutation(X.shape[0])
        Xm = lam[:, None] * X.astype(np.float32) + (1.0 - lam[:, None]) * X[idx].astype(np.float32)
        ym = lam * y.astype(np.float32) + (1.0 - lam) * y[idx].astype(np.float32)
        X_list.append(Xm); y_list.append(ym)
    ...
    print(f"[AUG] {kind}: {X.shape[0]} -> {X_aug.shape[0]} (dup={dup_factor}, mixup={'on' if mixup_alpha>0 else 'off'})")
    return X_aug, y_aug

def _stack_one(samples, kind="pd"):
    Xs, ys, refs, scls = [], [], [], []
    ykey, rkey, skey = ("sig_th","th_ref","th_scl") if kind=="pd" else ("sig_z","z_ref","z_scl")
    for s in samples:
        mask = np.isfinite(s["lam_th"]) & np.isfinite(s["lam_z"]) & np.isfinite(s[ykey])
        if not mask.any(): continue
        lam_th = s["lam_th"][mask]; lam_z = s["lam_z"][mask]; y = s[ykey][mask]
        Xs.append(np.stack([lam_th, lam_z], 1)); ys.append(y)
        refs.append(np.full_like(y, s[rkey], dtype=np.float32))
        scls.append(np.full_like(y, s[skey], dtype=np.float32))
    if not Xs:
        z = np.zeros
        return z((0,2),np.float32), z((0,),np.float32), z((0,),np.float32), np.ones((0,),np.float32)
    X = np.concatenate(Xs).astype(np.float32)
    y = np.concatenate(ys).astype(np.float32)
    ref = np.concatenate(refs).astype(np.float32)
    scl = np.concatenate(scls).astype(np.float32)
    return X, y, ref, scl

def _augment_arrays(X, y, ref, scl, dup=1, mixup=True, alpha=0.2, LMIN=0.6, LMAX=2.0, seed=42):
    rng = np.random.default_rng(seed)
    outX=[X]; outy=[y]; outR=[ref]; outS=[scl]
    # duplicate with tiny jitter in lambdas, then clip to the SAME bounds you use in physics
    for _ in range(max(0,dup)):
        j = rng.normal(0.0, 0.01, X.shape).astype(np.float32)
        Xj = np.clip(X + j, LMIN, LMAX)
        outX.append(Xj); outy.append(y.copy()); outR.append(ref.copy()); outS.append(scl.copy())
    # mixup (also mix ref/scale to keep normalized residual consistent)
    if mixup and X.shape[0] >= 2:
        i1 = rng.integers(0, X.shape[0], size=X.shape[0])
        i2 = rng.integers(0, X.shape[0], size=X.shape[0])
        lam = rng.beta(alpha, alpha, size=X.shape[0]).astype(np.float32)
        Xm = (lam[:,None]*X[i1] + (1-lam)[:,None]*X[i2]).astype(np.float32)
        Xm = np.clip(Xm, LMIN, LMAX)
        ym = lam*y[i1] + (1-lam)*y[i2]
        rm = lam*ref[i1] + (1-lam)*ref[i2]
        sm = np.maximum(lam*scl[i1] + (1-lam)*scl[i2], 1e-6)
        outX.append(Xm); outy.append(ym.astype(np.float32)); outR.append(rm.astype(np.float32)); outS.append(sm.astype(np.float32))
    X2 = np.concatenate(outX).astype(np.float32)
    y2 = np.concatenate(outy).astype(np.float32)
    r2 = np.concatenate(outR).astype(np.float32)
    s2 = np.concatenate(outS).astype(np.float32)
    return X2, y2, r2, s2

def build_dataset_with_meta(age_blob, do_aug=True, dup=1, mixup=True, alpha=0.2, LMIN=0.6, LMAX=2.0):
    X_th, y_th, r_th, s_th = _stack_one(age_blob["pd"], "pd")
    X_z,  y_z,  r_z,  s_z  = _stack_one(age_blob["fl"], "fl")
    if do_aug and X_th.shape[0]: X_th,y_th,r_th,s_th = _augment_arrays(X_th,y_th,r_th,s_th,dup,mixup,alpha,LMIN,LMAX)
    if do_aug and X_z.shape[0]:  X_z, y_z, r_z, s_z  = _augment_arrays(X_z, y_z, r_z, s_z, dup, mixup, alpha, LMIN, LMAX)
    # FAST caps (apply same selection to all arrays!)
    if FAST_DEV and X_th.shape[0] > MAX_POINTS_PER_AGE:
        sel = np.linspace(0, X_th.shape[0]-1, MAX_POINTS_PER_AGE, dtype=int)
        X_th,y_th,r_th,s_th = X_th[sel],y_th[sel],r_th[sel],s_th[sel]
    if FAST_DEV and X_z.shape[0] > MAX_POINTS_PER_AGE:
        sel = np.linspace(0, X_z.shape[0]-1, MAX_POINTS_PER_AGE, dtype=int)
        X_z,y_z,r_z,s_z = X_z[sel],y_z[sel],r_z[sel],s_z[sel]
    meta_th = {"ref": r_th, "scl": s_th} if X_th.size else None
    meta_z  = {"ref": r_z,  "scl": s_z } if X_z.size  else None
    return X_th, y_th, meta_th, X_z, y_z, meta_z
...
def main():
    ...
    for age in ages:
        ...
        X_th, y_th, meta_th, X_z, y_z, meta_z = build_dataset_with_meta(
         blob, do_aug=True, dup=1, mixup=True, alpha=0.2, LMIN=LMIN, LMAX=LMAX   
            )

        
        if AUG_ENABLE:
            X_th, y_th = augment_arrays(X_th, y_th, kind='theta')
            X_z,  y_z  = augment_arrays(X_z,  y_z,  kind='z')
