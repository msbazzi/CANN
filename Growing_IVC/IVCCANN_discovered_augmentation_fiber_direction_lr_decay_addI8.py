...
USE_LR_DECAY   = False      # turn on/off LR schedule
...
@tf.function(reduce_retracing=True)
def I8_from_stretches(lam_th, lam_z, phi):
    c2 = tf.cos(phi) ** 2
    s2 = tf.sin(phi) ** 2
    return lam_th**2 * c2 - lam_z**2 * s2
...
class PsiNet(keras.Model):
    ...
    def call(self, I1, I2, I4th, I4z, I4a, I4b, I8, training=False):
        def safe_expm1(z):
...
def _compute_joint_grads(model, xb, ytheta, yz):
    ...
    with tf.GradientTape() as tape_out:
        ...
        I8  = I8_from_stretches(lam_th, lam_z, phi)
        # dΨ/dI* via inner tape
        with tf.GradientTape() as tape_in:
            tape_in.watch([I1, I2, I4th, I4z, I4a, I4b, I8])
            psi = model(I1, I2, I4th, I4z, I4a, I4b, I8, training=True)

        dW_dI1, dW_dI2, dW_dI4th, dW_dI4z, dW_dI4a, dW_dI4b, dW_dI8 = tape_in.gradient(
            psi, [I1, I2, I4th, I4z, I4a, I4b, I8]
        )
        ...
        # [I8 ADD] cross-invariant contributions
        sig_th_aniso += 4.0 * ( dW_dI8 * (lam_th**2) * c2 )
        sig_z_aniso  += 4.0 * ( dW_dI8 * (lam_z**2) * (-s2) )
        ...
        if S0_W > 0.0:
            ...
            # invariants for the reference neighborhood (must be consistent!)
            I1r, I2r, I4thr, I4zr = invariants_from_stretches(u1, u2)
            I4ar = I4_fam_a(u1, u2, phi)
            I4br = I4_fam_b(u1, u2, phi)
            I8r  = I8_from_stretches(u1, u2, phi)  # <-- build I8r for u1/u2

            with tf.GradientTape() as t_ref:
                # watch all the ref invariants actually fed to the model
                t_ref.watch([I1r, I2r, I4thr, I4zr, I4ar, I4br, I8r])
                psi_r = model(I1r, I2r, I4thr, I4zr, I4ar, I4br, I8r, training=True)

            # get dΨ/dI* with a null-safe fallback (prevents "None values not supported")
            grads_r = t_ref.gradient(psi_r, [I1r, I2r, I4thr, I4zr, I4ar, I4br, I8r])
            d1, d2, d4t, d4z, d4a, d4b, d8 = [
                (g if g is not None else tf.zeros_like(x))
                for g, x in zip(grads_r, [I1r, I2r, I4thr, I4zr, I4ar, I4br, I8r])
            ]

            # stresses at the reference neighborhood
            ...
            sig_th_aniso_r = 4.0*( d4a*(u1**2)*c2r + d4b*(u1**2)*s2r ) \
                           + 4.0*( d8 *(u1**2)*c2r )          # I8 term
            sig_z_aniso_r  = 4.0*( d4a*(u2**2)*s2r + d4b*(u2**2)*c2r ) \
                           + 4.0*( d8 *(u2**2)*(-s2r) )       # I8 term
...
def predict_sigma_hat(model, X, mode="theta"):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        I8  = I8_from_stretches(lam_th, lam_z, phi) 
        psi = model(I1, I2, I4th, I4z, I4a, I4b, I8, training=False)

    dW_dI1, dW_dI2, dW_dI4th, dW_dI4z, dW_dI4a, dW_dI4b, dW_dI8 = tape.gradient(
        psi, [I1, I2, I4th, I4z, I4a, I4b, I8]
    )
    del tape
    ...
    c2 = tf.cos(phi)**2
    s2 = tf.sin(phi)**2
    sig_th_aniso = 4.0 * ( dW_dI4a * (lam_th**2) * c2 + dW_dI4b * (lam_th**2) * s2 ) +  4.0 * ( dW_dI8  * (lam_th**2) * c2 )
    sig_z_aniso  = 4.0 * ( dW_dI4a * (lam_z**2)  * s2 + dW_dI4b * (lam_z**2)  * c2 ) + 4.0 * ( dW_dI8  * (lam_z**2)  * (-s2) )
...
def main():
    ...
    def feature_count_for(model):
        ...
        branches += int(hasattr(model, "bI8"))
    ...
    for age in ages:
        ...
        with strategy.scope():
            ...
            I8  = I8_from_stretches(_lam_th, _lam_z, phi)
            _ = model(I1, I2, I4th, I4z, I4a, I4b, I8, training=False)
